...
func compareCodeSlices(t *testing.T, got, want []*terminology.Code) {
	t.Helper()
	if len(got) != len(want) {
...
func TestExpandValueSet(t *testing.T) {
	...
	provider, err := terminology.NewInMemoryFHIRProvider(getMainTestValueSets())
	if err != nil {
...
func TestExpandValueSetErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			version:             "",
			expectSpecificError: terminology.ErrCircularReference,
		},
	}
...
func TestExpandValueSetEdgeCases(t *testing.T) {
	...
	provider, err := terminology.NewInMemoryFHIRProvider(getEdgeCaseTestValueSets())
	if err != nil {
		t.Fatalf("Failed to create provider: %v", err)
	}
...
func TestExpandValueSetEdgeCasesErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			version:             "",
			expectSpecificError: terminology.ErrResourceNotLoaded,
		},
		{
...
func TestExpandValueSetWithSystemFilters(t *testing.T) {
	...
	provider, err := terminology.NewInMemoryFHIRProvider(getSystemFilterTestValueSets())
	if err != nil {
		t.Fatalf("Failed to create provider: %v", err)
	}
...
func TestProviderNotInitialized(t *testing.T) {
	tests := []struct {
		name        string
		testFunc    func(*terminology.LocalFHIRProvider) error
		expectError error
	}{
		{
			name: "ExpandValueSet on nil provider",
			testFunc: func(p *terminology.LocalFHIRProvider) error {
				_, err := p.ExpandValueSet("test", "")
				return err
			},
			expectError: terminology.ErrNotInitialized,
		},
		{
			name: "AnyInValueSet on nil provider",
			testFunc: func(p *terminology.LocalFHIRProvider) error {
				_, err := p.AnyInValueSet([]terminology.Code{}, "test", "")
				return err
			},
			expectError: terminology.ErrNotInitialized,
		},
	...
	var provider *terminology.LocalFHIRProvider
